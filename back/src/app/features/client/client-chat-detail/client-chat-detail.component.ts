...
@Component({
  selector: 'app-client-chat-detail',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './client-chat-detail.component.html',
  styleUrl: './client-chat-detail.component.scss'
})
export class ClientChatDetailComponent implements OnInit, OnDestroy {
  ...
  private loadMessages(conversationId: number): void {
    ...
    this.messageService.findByConversation(conversationId)
      .pipe(
        tap(messages => {
          console.log('Messages client chargés :', messages);
          this.messagesSubject.next(messages);
        }),
        catchError(error => {
          console.error('Erreur chargement messages client', error);
  ...
  private listenToMessages(conversationId: number): void {
    ...
    this.messageSocketService.messages$
      .pipe(
        tap(messages => {
          console.log('Chat complet reçu côté client via WebSocket :', messages);
          this.messagesSubject.next(messages);
        }),
        takeUntil(this.destroy$)
      )
